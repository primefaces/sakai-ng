...
import { NotaVenta_Detalles } from '../../models/detalleventa';
...
import { EstadoCuentaDevolucion } from '../../../cliente/models/EstadoCuenta';
...
@Component({
  standalone: false,
  selector: 'app-nota-venta-estado-cliente',
  templateUrl: './nota-venta-estado-cliente.component.html',
  styleUrl: './nota-venta-estado-cliente.component.scss',
  providers: [UtilsService]
})
export class NotaVentaEstadoClienteComponent  implements OnChanges{
  ...
  NotasDetalles: NotaVenta_Detalles[] = [];
  ...
  cargarEstadoCuenta() {
    this.ClienteService.EstadoClienteNotaVenta(this.cliente.id, this.gestion.id).subscribe(response => {
      this.EstadoCuenta = response;
    });
  }

  cargarEstadoCuentaDevolucion() {
    this.ClienteService.EstadoClienteNotaDevolucion(this.cliente.id, this.gestion.id).subscribe(response => {
      this.EstadoCuentaDevolucion = response;
    });
  }

  generateEstadoCuentaPDF() {
    const reqdevolucion = this.ClienteService.EstadoCuentaPDFDevolucion(this.cliente.id, this.gestion.id);
    reqdevolucion.subscribe({
      next: (response) => {
        this.NotasDetallesDevolucion = response;
      },
      complete: () => {
        const req = this.ClienteService.EstadoCuentaPDF(this.cliente.id, this.gestion.id);
        req.subscribe({
          next: (response) => {
  ...
  informePDF2() {
        ...
        table: {
          ...
          body: [
            [
              ...
              {
                text: `ESTADO DE CUENTA ${this.cliente.nombre} ${this.cliente.paterno} ${this.cliente.materno}`,
                alignment: 'center',
                bold: true,
                fontSize: 14,
                margin: [0, 10, 0, 5],
            ...
            [
              {},
              {
                text: `AGROAISA-DA (Fertilizantes-Agroquímicos)   Campaña: ${this.gestion.nombre_campania} (${this.gestion.anio})   Moneda: DÓLARES`,
                alignment: 'center',
                fontSize: 10,
                margin: [0, 0, 0, 10],
                border: [true, true, true, true]
    ...
    this.NotasDetalles.forEach(nota => {
      tableBody.push([
        {
          text: nota.fecha + (nota.venta_credito == true ? ' (CREDITO)' : ' (CONTADO)') + (nota.firma ? ' (FIRMADO)' : '' + (nota.lugar ? ' (' + nota.lugar + ')' : '')),
          colSpan: 8,
          bold: true,
          fillColor: '#eeeeee',
          alignment: 'center',
    ...
    const total = this.NotasDetalles.reduce((acc, nota) => {
      const subtotalNota = nota.detalles_venta.reduce((sum, d) => sum + Number(d.subtotal), 0);
      ...
      { text: '\nESTADO X COBRAR', bold: true },
      {
        table: {
          ...
          body: [
            [
              { text: 'NOTA', bold: true },
              { text: 'CAP.TOTAL VENTA', bold: true },
    ...
    const fechaHora = `${fecha.getFullYear()}-${(fecha.getMonth() + 1)
      .toString()
      .padStart(2, '0')}-${fecha.getDate().toString().padStart(2, '0')}_${horas12}-${minutos}_${ampm}`;
    
    const fileName = `ESTADO_CUENTA_VENTAS_${this.gestion.nombre_campania}(${this.gestion.anio})_${this.cliente.nombre}_${this.cliente.paterno}_${this.cliente.materno}_${fechaHora}.pdf`;
