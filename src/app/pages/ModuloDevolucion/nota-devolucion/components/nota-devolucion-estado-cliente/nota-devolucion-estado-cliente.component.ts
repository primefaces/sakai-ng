...
import { NotaDevolucion_Detalles } from '../../models/detalledevolucion';
...
import { EstadoCuentaDevolucion } from '../../../../ModuloVenta/cliente/models/EstadoCuenta';

pdfMake.vfs = pdfFonts.vfs;

@Component({
  standalone: false,
  selector: 'app-nota-devolucion-estado-cliente',
  templateUrl: './nota-devolucion-estado-cliente.component.html',
  styleUrl: './nota-devolucion-estado-cliente.component.scss',
  providers: [UtilsService]
})
export class NotaDevolucionEstadoClienteComponent  implements OnChanges{

  Configuracion: Configuracion = {} as Configuracion;
  EstadoCuenta: EstadoCuentaDevolucion = {} as EstadoCuentaDevolucion;
  NotasDetalles: NotaDevolucion_Detalles[] = [];
  ...
  generateEstadoCuentaPDF() {
    const req = this.ClienteService.EstadoCuentaPDFDevolucion(this.cliente.id, this.gestion.id);
    req.subscribe({
      next: (response) => {
  ...
  informePDF() {
    ...
    content.push({
      table: {
        ...
        body: [
          [
            { text: 'TOTAL DEVOLUCION', bold: true },
            { text: `$ ${total.toFixed(2)}`, fillColor: '#00FF66', bold: true }
          ]
        ]
      },
      layout: {
    ...
    const fileName = `ESTADO_CUENTA_DEVOLUCION_${this.gestion.nombre_campania}(${this.gestion.anio})_${this.cliente.nombre}_${this.cliente.paterno}_${this.cliente.materno}_${fechaHora}.pdf`;
